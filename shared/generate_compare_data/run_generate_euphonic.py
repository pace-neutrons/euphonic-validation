...
for cut in quartz_cuts:
    for temp in quartz_temps:
        sqw_main(['quartz', cut, '--temp', temp,
                  '--ofig', os.path.join(get_dir('quartz', cut=cut, code='euphonic'),
                                         'euphonic_sqw_'+ temp + 'K.pdf')])
        sqw_main(['quartz', cut, '--temp', temp, '--freqs'])
...
for temp in lzo_temps:
    for cut in lzo_cuts:
        ...
        sf_main(['lzo', cut,
                 '--dw', get_euphonic_fpath('lzo', 'euphonic', 'dw', temp, from_fc=False, grid=lzo_grid),
                 '--freqs'])
# Generate OClimax S(Q,w) map for LZO
for cut in lzo_cuts:
    for temp in lzo_temps:
        sqw_main(['lzo', cut, '--temp', temp,
                  '--ofig', os.path.join(get_dir('lzo', cut=cut, code='euphonic'),
                                         'euphonic_sqw_'+ temp + 'K.pdf')])
        sqw_main(['lzo', cut, '--temp', temp, '--freqs'])
...
for cut in nb_cuts:
    for temp in nb_temps:
        sqw_main(['nb', cut, '--temp', temp,
                  '--ofig', os.path.join(get_dir('nb', cut=cut, code='euphonic'),
                                         'euphonic_sqw_'+ temp + 'K.pdf')])
        sqw_main(['nb', cut, '--temp', temp, '--freqs'])
